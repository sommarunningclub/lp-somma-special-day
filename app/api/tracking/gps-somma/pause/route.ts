...
export async function POST(request: NextRequest) {
  ...
  if (session.status !== 'running') return NextResponse.json({ error: 'Sessão não está ativa.' }, { status: 409 })

  const { error } = await trackingDb()
    .from('gps_tracking_sessions')
    .update({ status: 'paused', paused_at: new Date().toISOString() })
    .eq('id', session.id)
  if (error) return NextResponse.json({ error: 'Falha ao pausar.' }, { status: 500 })
