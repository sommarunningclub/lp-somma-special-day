...
export default function ParticipantForm({ maxPhotos = 2 }: { maxPhotos?: number }) {
  ...
  async function onSubmit(data: RegistroInput) {
    ...
    setCpfExiste(false)
    if (fotos.length < 1) {
      setErro('Envie pelo menos uma foto do look.')
      return
    }
    try {
      const res = await fetch('/api/concurso/inscricao', {
        ...
        body: JSON.stringify({ ...data, fotos, website }),
      })
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5" onChange={() => track('contest_registration_started')}>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <input {...register('display_name')} className={input} placeholder="Como vai aparecer no mural" />
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <input inputMode="tel" {...register('whatsapp')} className={input} placeholder="(61) 99999-0000" />
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div>
          ...
          <input {...register('city')} className={input} placeholder="Brasília, DF" />
        </div>
        <div>
          <label className={label}>Título do look *</label>
          <input {...register('look_title')} className={input} placeholder="Ex: Caipira Raiz" />
          {errors.look_title && <p className={err}>{errors.look_title.message}</p>}
        </div>
      </div>

      <div>
        <label className={label}>Descrição do look (opcional)</label>
        <textarea rows={3} maxLength={500} {...register('look_description')} className={`${input} resize-none`} placeholder="Conta a inspiração do seu look..." />
      </div>

      <div>
        <label className={label}>Fotos do look *</label>
        <PhotoUploader value={fotos} onChange={setFotos} max={maxPhotos} />
      </div>
      ...
      <p className="text-center font-dm text-xs text-somma-black/45">
        Vamos te enviar um código por e-mail pra você acessar e gerenciar sua inscrição depois.
      </p>
    </form>
  )
}
