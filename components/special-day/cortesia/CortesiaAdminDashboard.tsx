...
export default function CortesiaAdminDashboard({ leads }: { leads: CortesiaLead[] }) {
  ...
  function copyAllTsv() {
    const tsv = [TSV_HEADER.join('\t'), ...filtered.map(rowToTsv)].join('\n')
    copy(tsv, 'all-tsv')
  }

  function downloadCsv() {
    const csv = [TSV_HEADER, ...filtered.map(rowValues)]
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border-4 border-dashed border-somma-black/20 bg-white/60 px-5 py-16 text-center font-dm text-somma-black/50">
          {leads.length === 0 ? 'Nenhum cadastro na cortesia ainda.' : 'Nenhum resultado para a busca.'}
        </div>
      ) : (
        <ul className="grid gap-4">
          {filtered.map((l, i) => (
            <li
              key={l.id}
              className="rounded-2xl border-4 border-somma-black bg-white p-4 shadow-[5px_5px_0_#0a0a0a] sm:p-5"
            >
              <div className="mb-4 flex flex-col gap-3 border-b-2 border-somma-black/10 pb-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-somma-orange font-bebas text-lg text-somma-cream">
                    {i + 1}
                  </span>
                  <div className="min-w-0">
                    <h2 className="truncate font-bebas text-2xl tracking-wide text-somma-black">{l.nome || '—'}</h2>
                    <p className="font-dm text-xs text-somma-black/45">Cadastrado em {l.created_at_fmt}</p>
                  </div>
                </div>
